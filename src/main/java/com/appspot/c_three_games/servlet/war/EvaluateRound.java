    ...
    TxResult<Game> result = ofy().transact(new Work<TxResult<Game>>() {
      ...
      public TxResult<Game> run() {
        ...
        switch (game.getState()) {
          ...
          case ROUNDOVER:
            return new TxResult<>(new BadRequestException("Round has already been evaluated, start a new round"));
          case EVALUATING:
          ...
          switch (players.get(i).getState()) {
            ...
            case WON:
              String message = "player " + players.get(i).getNum() + "/" + players.get(i).getId() + " is in state: "
                + players.get(i).getState() + ". expected PLAYED, NOTINWAR, LOST";
        ...
        // create an array of cards being played (null = no card played this round (not in round/lost))
        ...
        if (winners.size() > 1) {
          ...
          msgs.add(new Message.Builder().setMessageType(Message.ChannelType.WAR).setGame(game).addPlayers(winners)
            .build());
          msgs.add(new Message.Builder().setMessageType(Message.PlayerType.WAR).setGame(game).addPlayers(winners)
            .build());
        }
        // 1 winner
        if (winners.size() == 1) {
          ...
          msgs.add(new Message.Builder().setMessageType(Message.ChannelType.PLAYER_WON_ROUND).setGame(game)
            .addPlayer(winner).addCards(wonCards).build());
          msgs.add(new Message.Builder().setMessageType(Message.PlayerType.PLAYER_WON_ROUND).setGame(game)
            .addPlayer(winner).addCards(wonCards).build());
          game.setState(Game.State.ROUNDOVER);
          queue.add(
            ofy().getTransaction(),
            TaskOptions.Builder.withUrl("/tasks/StartNewRound").param("gameId", gameId.toString())
              .method(TaskOptions.Method.POST).countdownMillis(wonCards.size() * 1000 + 2000 + 600));
        }
